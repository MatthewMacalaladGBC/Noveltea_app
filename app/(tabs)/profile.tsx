...
export default function ProfileScreen() {
  ...
  // Load avatar on mount — works even if user is null (uses last known userId)
  useEffect(() => {
    async function loadAvatar() {
      if (user) {
        await AsyncStorage.setItem('noveltea_last_user_id', String(user.userId));
        const uri = await AsyncStorage.getItem(`noveltea_avatar_${user.userId}`).catch(() => null);
        if (uri) setAvatarUri(uri);
      } else {
        const lastId = await AsyncStorage.getItem('noveltea_last_user_id').catch(() => null);
        if (lastId) {
          const uri = await AsyncStorage.getItem(`noveltea_avatar_${lastId}`).catch(() => null);
          if (uri) setAvatarUri(uri);
        }
      }
    }
      ...
      async function fetchLists() {
        try {
          ...
          const results = await Promise.all(
            userLists.map((l) =>
              listsApi
                .getListItems(l.listId, token!)
                .then((items) => ({ listId: l.listId, items })),
            ),
          );
        ...
        <View style={styles.statsContainer}>
          <Pressable
            ...
            onPress={() =>
              router.push({
                pathname: "/follows/[id]",
                params: { id: String(user.userId), tab: "followers" },
              } as any)
            }
          >
          ...
          <Pressable
            ...
            onPress={() =>
              router.push({
                pathname: "/follows/[id]",
                params: { id: String(user.userId), tab: "following" },
              } as any)
            }
          >
            <Text variant="headlineMedium" style={[styles.statNumber, { color: theme.colors.onBackground }]}>
          ...
          <Pressable
            ...
            onPress={() =>
              router.push({
                pathname: "/user-reviews/[username]",
              ...
              <View key={list.listId} style={styles.listRow}>
                <View style={styles.listNameRow}>
                  <Text
                    ...
                    onPress={() =>
                      router.push({ pathname: "/list/[id]", params: { id: String(list.listId) } } as any)
                    }
                  >
                    {list.title} ›
                  </Text>
...
const styles = StyleSheet.create({
  ...
  bio: { marginTop: 8, opacity: 0.6, textAlign: "center", fontStyle: "italic", paddingHorizontal: 16 },
  statsContainer: {
    flexDirection: "row", flexWrap: "wrap", justifyContent: "space-around",
    paddingVertical: 24, paddingHorizontal: 20, rowGap: 16,
  },
  statItem: { alignItems: "center", minWidth: 90 },
  statNumber: { fontWeight: "bold", marginBottom: 4 },
  statLabel: { fontSize: 12, opacity: 0.7 },
  divider: { marginVertical: 8 },
  myListsSection: { paddingVertical: 8 },
  myListsHeader: {
    flexDirection: "row", justifyContent: "space-between", alignItems: "center",
    paddingHorizontal: 16, marginBottom: 4,
  },
  myListsTitle: { fontWeight: "bold" },
  createListBtn: {
    marginHorizontal: 16, marginBottom: 4, paddingVertical: 10,
    borderRadius: 8, borderWidth: 1, alignItems: "center",
  },
  createListBtnText: { fontSize: 14, fontWeight: "600" },
  seeAllLink: { fontSize: 22 },
  listRow: { marginTop: 12 },
  listNameRow: { flexDirection: "row", alignItems: "baseline", gap: 8, paddingHorizontal: 16, marginBottom: 8 },
  listName: { fontWeight: "600" },
  bookCountBadge: { fontSize: 12, opacity: 0.55 },
  horizontalList: { paddingHorizontal: 16, paddingBottom: 4 },
  listLoader: { marginLeft: 16, marginBottom: 8 },
  emptyText: { paddingHorizontal: 16, paddingBottom: 8, opacity: 0.6, fontSize: 14 },
  settingsContainer: { paddingVertical: 8 },
  actionContainer: { paddingHorizontal: 20, paddingVertical: 24 },
